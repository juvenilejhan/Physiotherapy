...
export default function AdminTelehealthPage() {
  ...
  useEffect(() => {
    ...
    if (session?.user?.role && !['SUPER_ADMIN', 'CLINIC_MANAGER', 'DOCTOR'].includes(session.user.role)) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">Telehealth Sessions</h2>
        <p className="text-muted-foreground">
          Manage video consultations and remote sessions.
        </p>
      </div>

      <div className="flex items-center justify-center min-h-[400px] border-2 border-dashed rounded-lg">
        <div className="text-center">
          <p className="text-lg font-medium">Telehealth Module</p>
