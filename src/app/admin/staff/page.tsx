...
export default function StaffManagementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editMode ? `/api/admin/staff/${selectedStaff?.user.id}` : '/api/admin/staff';
      ...
      if (response.ok) {
        toast.success(editMode ? 'Staff member updated successfully' : 'Staff member added successfully');
        setDialogOpen(false);
        resetForm();
        fetchStaff();
      } else {
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                {editMode ? 'Update staff member information' : 'Add a new staff member to the clinic'}
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit}>
              ...
              <DialogFooter>
                <Button type="submit">{editMode ? 'Update' : 'Add Staff'}</Button>
              </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
              ...
              {filteredStaff.length === 0 ? (
                ...
                filteredStaff.map((staffMember) => (
                  <TableRow key={staffMember.id}>
                    ...
                    <TableCell>
                      <Badge variant={getRoleBadgeVariant(staffMember.user.role)}>
                        {staffMember.user.role.replace('_', ' ')}
                      </Badge>
                    </TableCell>
                    <TableCell>{staffMember.specialty || '-'}</TableCell>
                    <TableCell>
                      {staffMember.experience ? `${staffMember.experience} years` : '-'}
                    </TableCell>
                    <TableCell>
